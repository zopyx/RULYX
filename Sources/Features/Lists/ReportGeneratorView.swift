        ...
        for entry in log.prefix(10) {
            lines.append(
                loc("report.doc_activity_line")
                    ...
                    .replacingOccurrences(of: "{summary}", with: entry.summary)
                    .replacingOccurrences(of: "{date}", with: entry.createdAt.formatted(date: .abbreviated, time: .shortened))
            )
        }
