            ...
            VStack(spacing: 12) {
                ...
                HStack(spacing: 8) {
                    actionButton(loc("profile.block"), isActive: viewer?.isBlocking == true) {
                        await profileVM.toggleBlock(account: accountStore.activeAccount!, appPassword: accountStore.activeAccount.flatMap { accountStore.appPassword(for: $0) } ?? "", using: container.liveClient)
                    }
                    actionButton(loc("profile.mute"), isActive: viewer?.muted == true) {
                        await profileVM.toggleMute(account: accountStore.activeAccount!, appPassword: accountStore.activeAccount.flatMap { accountStore.appPassword(for: $0) } ?? "", using: container.liveClient)
                    }
                    actionButton(loc("profile.follow"), isActive: viewer?.isFollowing == true) {
                        await profileVM.toggleFollow(account: accountStore.activeAccount!, appPassword: accountStore.activeAccount.flatMap { accountStore.appPassword(for: $0) } ?? "", using: container.liveClient)
                    }
                }
            }
        }
